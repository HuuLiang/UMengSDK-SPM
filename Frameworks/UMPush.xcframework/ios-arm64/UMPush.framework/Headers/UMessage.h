...
typedef void (^UMPLiveActivityTokenCompletion)(BOOL isSuccess, NSInteger seq, NSError * _Nullable error);
...
/**
 上报 liveActivity PushToStartToken

 @param activityAttributes   实时活动的属性，即当前ActivityAttributes结构体名称
 @param pushToStartToken     推送的 pushToStartToken
 @param completion     用于回调返回对应的状态码
 @param seq     请求时传入的序列号
 */
+ (void)registerLiveActivity:(NSString *_Nonnull)activityAttributes
            pushToStartToken:(NSString *_Nullable)pushToStartToken
                  completion:(UMPLiveActivityTokenCompletion _Nullable )completion
                         seq:(NSInteger)seq;
/**
 上报 liveActivity PushToken

 @param liveActivityId   标识某一个 liveActivity，业务id，用于绑定token的业务关系
 @param pushToken     对应该 liveactivity 的 pushToken
 @param completion     用于回调返回对应的状态码
 @param seq     请求时传入的序列号
 */
+ (void)registerLiveActivity:(NSString *_Nonnull)liveActivityId
                   pushToken:(NSString  * _Nullable)pushToken
                  completion:(UMPLiveActivityTokenCompletion _Nullable )completion
                         seq:(NSInteger)seq;
@end
